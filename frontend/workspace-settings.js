(() => {
  ...
  async function request(method='GET',body,scope=workspaceId,full=false){
    ...
    const response=await fetch('/api/log-sources'+(method==='GET'?'?workspace='+encodeURIComponent(scope):''),{method,headers:body?{'Content-Type':'application/json'}:{},body:body?JSON.stringify(body):undefined,signal:AbortSignal.timeout(35000)});
  ...
  function open(source=null){
    ...
    field('directory').value=source?.logDirectory||'';field('path').value=source?.logPath||'';field('enabled').checked=source?.enabled??true;field('password').value='';$('#source-form-feedback').textContent='';passwordHint();$('#source-dialog').showModal();
  }
  ...
  $('#source-form').addEventListener('submit',async e=>{
    ...
    const scope=sourceScope,body={action:'save',workspace:scope,id:editing?.id,name:field('name').value,service:field('service').value,host:field('host').value,port:Number(field('port').value),username:field('username').value,password:field('password').value,environment:field('environment').value,logDirectory:field('directory').value,logPath:field('path').value,enabled:field('enabled').checked};
  ...
  $('#remote-source-list').addEventListener('click',async e=>{
    ...
    $('#source-feedback').textContent=testing?'正在验证 SSH 登录与日志权限，最长约 30 秒…':'正在保存配置…';
  ...
  function openLogQuery(source){
    querySource=source;queryScope=workspaceId;$('#log-query-form').reset();$('#log-query-source').textContent=activeWorkspace.name+' / '+source.name+' · '+source.logPath;
  ...
  $('#log-query-form').addEventListener('submit',async e=>{
    ...
    const scope=queryScope;setBusy(true);$('#log-query-feedback').textContent='正在读取服务器日志，最长约 30 秒…';$('#log-query-output').textContent='';
    try{
      ...
      $('#log-query-feedback').textContent=data.result.message+' · 最多 20 处匹配，上下文各 200 行 · 耗时 '+(data.result.durationMs/1000).toFixed(1)+' 秒'+(data.saveWarning?'；'+data.saveWarning:'');
