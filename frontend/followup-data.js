...
(() => {
  function append(report,input){
    ...
    if(!turn||typeof turn.id!=='string'||!/^[a-zA-Z0-9_-]{1,100}$/.test(turn.id)||typeof turn.question!=='string'||!turn.question.trim()||turn.question.length>PayTraceLimits.question||typeof turn.text!=='string'||!turn.text.trim()||turn.text.length>PayTraceLimits.answer||typeof turn.model!=='string'||turn.model.length>200||!Number.isFinite(Date.parse(turn.createdAt)))throw Error('追问记录格式无效');
  ...
  function context(report){
    ...
    for(let i=turns.length-1;i>=0&&recent.length<PayTraceLimits.historyTurns;i--){
      ...
      size+=t.question.length+t.text.length;imageCount+=images.length;if(size>PayTraceLimits.history||imageCount>10)break;recent.unshift({question:t.question,text:t.text,...(images.length?{images}:{})});
    }
