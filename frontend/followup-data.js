...
(() => {
  function append(report,input){
    ...
    if(existing){if(existing.question!==turn.question||existing.text!==turn.text||JSON.stringify(existing.logQuery)!==JSON.stringify(turn.logQuery))throw Error('追问记录冲突，请重新打开报告');return report;}
    if(input.expectedCount!==turns.length)throw Error('对话已更新，请重新打开报告后追问');
    report.followups=[...turns,{id:turn.id,question:turn.question,text:turn.text,model:turn.model,createdAt:turn.createdAt,...(turn.logQuery?{logQuery:structuredClone(turn.logQuery)}:{})}];return report;
  }
