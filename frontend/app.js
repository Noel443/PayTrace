...
function realFeedbackPanel(r){
  const status=r.feedback?.status||'未反馈';
  const options=[['已解决','已确认原因，记录处理结果','✓'],['需要开发介入','留下排查进展与待协助事项','↗'],['判断不正确','纠正结论，补充事实依据','!']];
  return '<section class="feedback-bar resolution-panel" aria-labelledby="resolution-title">'+
    '<div class="resolution-heading"><div><h2 id="resolution-title">处理结论</h2><p>排查之后，留下处理结果，方便后续回看与交接。</p></div><span class="tag" id="resolution-badge">'+esc(status==='未反馈'?'待反馈':status)+'</span></div>'+
    '<select id="real-feedback" hidden aria-label="处理结果">'+['未反馈',...options.map(o=>o[0])].map(s=>'<option'+(status===s?' selected':'')+'>'+s+'</option>').join('')+'</select>'+
    '<div class="resolution-options" role="group" aria-label="选择处理结果">'+options.map(([s,description,icon])=>'<button type="button" class="resolution-option" data-resolution="'+s+'" aria-pressed="'+(status===s)+'"><span class="resolution-icon" aria-hidden="true">'+icon+'</span><span><strong>'+s+'</strong><small>'+description+'</small></span><span class="resolution-check" aria-hidden="true">✓</span></button>').join('')+'</div>'+
    '<div class="resolution-note-heading"><label for="real-note" id="resolution-note-label">处理说明</label><button type="button" class="resolution-link" id="resolution-outline">插入填写提纲</button></div>'+
    '<textarea id="real-note" rows="3" maxlength="2000" aria-describedby="resolution-help resolution-count">'+esc(r.feedback?.note||'')+'</textarea>'+
    '<div class="resolution-note-meta"><span id="resolution-help">选择处理结果后，补充关键进展与依据。</span><span id="resolution-count"></span></div>'+
    '<div class="resolution-footer"><span id="resolution-save-state" role="status" aria-live="polite"></span><div class="resolution-buttons"><button class="text-button" type="button" id="resolution-reset">撤销修改</button><button class="primary" type="button" id="real-save">保存反馈</button></div></div>'+
    '<div class="resolution-retry"><div><strong>有新的日志或处理进展？</strong><p>重新查询最新日志并生成一份新报告，当前报告会保留。</p></div><button class="text-button" type="button" id="real-retry">重新查询并分析 <span aria-hidden="true">↗</span></button></div></section>';
}
function bindRealFeedback(r){
  const panel=$('.resolution-panel'),select=$('#real-feedback'),note=$('#real-note'),save=$('#real-save'),message=$('#resolution-save-state');
  let saved={status:r.feedback?.status||'未反馈',note:r.feedback?.note||''},saving=false;
  const guidance={
    '已解决':['处理结果','说明已确认的原因、采取的处理措施，以及如何验证问题已解决。','确认原因：\n处理措施：\n验证结果：'],
    '需要开发介入':['交接说明','说明异常现象、已排查的内容，以及需要研发协助确认的事项。','异常现象：\n已排查内容：\n待协助事项：'],
    '判断不正确':['纠正说明','指出不准确的结论，并补充实际情况或可以核验的证据。','原判断的问题：\n实际情况：\n核验依据：']
  };
  const dirty=()=>select.value!==saved.status||note.value!==saved.note;
  const sync=()=>{
    const choice=guidance[select.value],changed=dirty();
    panel.querySelectorAll('[data-resolution]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.resolution===select.value)));
    $('#resolution-note-label').textContent=choice?.[0]||'处理说明';
    note.placeholder=choice?.[1]||'先选择处理结果，再补充说明…';
    $('#resolution-help').textContent=select.value==='需要开发介入'?'此处记录交接信息；请通过现有协作渠道联系研发。':select.value==='判断不正确'?'请尽量提供事实依据，便于后续复核。':'建议记录关键原因、处理措施和验证结果。';
    $('#resolution-count').textContent=note.value.length+' / 2000';
    $('#resolution-outline').disabled=saving||!choice||!!note.value.trim();
    $('#resolution-reset').disabled=saving||!changed;
    save.disabled=saving||busy||!choice||!changed;
    message.textContent=saving?'正在保存…':changed?'有未保存的修改':saved.status==='未反馈'?'尚未记录处理结果':'反馈已保存';
    save.textContent=saving?'保存中…':'保存反馈';
  };
  panel.syncFeedback=sync;
  panel.hasFeedbackChanges=dirty;
  panel.querySelectorAll('[data-resolution]').forEach(b=>b.onclick=()=>{if(saving)return;select.value=b.dataset.resolution;sync()});
  note.addEventListener('input',sync);
  $('#resolution-outline').onclick=()=>{if(!note.value.trim()&&guidance[select.value]){note.value=guidance[select.value][2];sync();note.focus()}};
  $('#resolution-reset').onclick=()=>{select.value=saved.status;note.value=saved.note;sync()};
  save.onclick=async()=>{
    if(saving||busy||!guidance[select.value]||!dirty())return;
    const payload={status:select.value,note:note.value},scope=r.workspaceId||workspaceId;
    saving=true;sync();note.disabled=true;panel.querySelectorAll('[data-resolution]').forEach(b=>b.disabled=true);
    try{
      const updated=await api('/investigations/'+r.id+'/feedback','POST',payload,scope);
      saved={status:updated.feedback?.status||payload.status,note:updated.feedback?.note??payload.note};
      if(workspaceId===scope&&current?.id===r.id){current={...current,feedback:updated.feedback};r.feedback=updated.feedback;}
      panel.querySelector('#resolution-badge')?.replaceChildren(document.createTextNode(saved.status));
      saving=false;sync();toast('处理反馈已保存');
    }catch(error){saving=false;sync();message.textContent='保存失败：'+error.message+'，填写内容已保留。';}
    finally{note.disabled=false;panel.querySelectorAll('[data-resolution]').forEach(b=>b.disabled=false);}
  };
  $('#real-retry').onclick=()=>{
    if(busy||saving)return;
    if(dirty()){message.textContent='请先保存反馈或撤销修改，再重新查询。';return;}
    $('#question').value=[r.searchQuery,r.question].filter(Boolean).join('：');window.questionImages?.set(r.images);$('#investigate-form').requestSubmit();
  };
  sync();
}
