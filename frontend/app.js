...
let transactions=[], current=null, feedbackStatus='', busy=false, historyRows=[];
...
async function runAi(report){
  const reportWorkspace=report.workspaceId||'card';
  const button=$('#analyze-ai'), progress=$('#ai-progress');
  if(location.protocol==='file:'){progress.textContent='请先运行 npm start，再打开 http://127.0.0.1:5173 使用真实 AI';return}
  button.disabled=true;progress.textContent='正在请求真实模型，最长等待约 60 秒…';
  const started=performance.now(), revision=report.revision??0;
  try{
    const knowledge=await api('/knowledge','GET',undefined,reportWorkspace);
    const response=await fetch('/api/ai/analyze',{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(65000),body:JSON.stringify({report,markdown:knowledge.markdown})});
    let result;try{result=await response.json()}catch{throw Error('AI 服务返回异常，请检查本地服务是否已重启')}
    if(!response.ok)throw Error(result.message||'AI 分析失败');
    result.durationMs=Math.round(performance.now()-started);
    result.revision=revision;
    const updated=await api('/investigations/'+report.id+'/ai','POST',result,reportWorkspace);
    if(workspaceId===reportWorkspace&&current?.id===report.id){current=updated;const note=$('#feedback-note');if(note){const draft=note.value,selected=feedbackStatus;renderReport();$('#feedback-note').value=draft;feedbackStatus=selected;document.querySelectorAll('[data-feedback]').forEach(b=>b.classList.toggle('selected',b.dataset.feedback===selected))}else renderReport()}
    toast(window.workspaceData.get(reportWorkspace).name+'：AI 分析已完成并保存');
  }catch(error){progress.textContent=error.name==='TimeoutError'?'模型请求超时，请稍后重试':error.message;toast('AI 分析未完成，原有报告已保留')}
...
function bindInvestigation(r){
  document.querySelectorAll('[data-evidence]').forEach(b=>b.addEventListener('click',()=>{const target=document.getElementById('evidence-'+b.dataset.evidence);if(target){target.open=true;target.scrollIntoView({behavior:'smooth',block:'center'});target.querySelector('summary').focus()}}));
...
function aiTextWithReferences(r){
  ...
  return esc(r.ai.text).replace(/\[(E\d+)\]/g,(match,id)=>ids.has(id)?`<button type="button" class="evidence-link ai-citation" data-evidence="${id}" aria-label="查看证据 ${id}">${match}</button>`:match);
}
