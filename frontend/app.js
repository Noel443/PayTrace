...
if(transactions.length)choose(transactions[0].id);else{$('#transaction-id').value='';$('#question').value='';$('#scenario-list').innerHTML='<div class="workspace-empty"><h3>输入流水号或订单号，开始 AI 排查</h3><p>配置并启用日志数据源和 AI 模型后，点击开始排查将自动查询当前空间的日志，并结合已保存的业务文档分析问题。</p><button type="button" class="primary" id="setup-workspace">配置数据源与业务文档</button></div>';$('#setup-workspace').onclick=()=>view('settings');}
...
$('#investigate-form').addEventListener('submit',async e=>{
  ...
  try{
    ...
    const response=await fetch('/api/investigations/stream',{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({workspace:scope,workspaceName:activeWorkspace.name,transactionId:$('#transaction-id').value,question:$('#question').value,markdown:knowledge.markdown||''})});
...
function renderRealReport(r){
  ...
  renderFollowup(r);
  $('#real-retry').onclick=()=>{$('#transaction-id').value=r.transaction.id;$('#question').value=r.question;$('#investigate-form').requestSubmit();};
