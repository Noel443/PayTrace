...
if(transactions.length)choose(transactions[0].id);else{$('#transaction-id').value='';$('#question').value='';$('#scenario-list').innerHTML='<div class="workspace-empty"><h3>工作空间已就绪，等待接入业务数据</h3><p>先添加服务器日志数据源，并完善业务文档。可在服务配置中查询真实日志、配置模型并运行项目 AI 分析；真实交易查询及自动生成报告尚未接入。</p><button type="button" class="primary" id="setup-workspace">配置数据源与业务文档</button></div>';$('#setup-workspace').onclick=()=>view('settings');}
...
$('#investigate-form').addEventListener('submit',async e=>{e.preventDefault();if(!transactions.length){toast('当前空间可在服务配置中查询服务器日志；交易查询与自动生成报告尚未接入');return;}if(busy)return;aiJob?.controller.abort();busy=true;$('#run').disabled=true;$('#run').textContent='排查中…';$('#loading').hidden=false;$('#error').hidden=true;$('#result').hidden=true;$('#empty').hidden=true;try{current=await api('/investigations','POST',{transactionId:$('#transaction-id').value,question:$('#question').value});renderReport()}catch(e){$('#error').textContent=e.message;$('#error').hidden=false;$('#empty').hidden=false}finally{busy=false;$('#run').disabled=false;$('#run').innerHTML='开始排查 <span>↗</span>';$('#loading').hidden=true}});
