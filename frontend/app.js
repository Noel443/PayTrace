...
if(transactions.length)choose(transactions[0].id);else{$('#transaction-id').value='';$('#question').value='';$('#scenario-list').innerHTML='<div class="workspace-empty"><h3>输入流水号或添加截图，开始 AI 排查</h3><p>配置并启用 AI 模型后，可上传截图提问；也可配置日志数据源，按流水号查询日志，并结合业务文档分析问题。</p><button type="button" class="primary" id="setup-workspace">配置数据源与业务文档</button></div>';$('#setup-workspace').onclick=()=>view('settings');}
...
$('#investigate-form').addEventListener('submit',async e=>{
  ...
  try{
    ...
    const response=await fetch('/api/investigations/stream',{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({workspace:scope,workspaceName:activeWorkspace.name,transactionId:$('#transaction-id').value,question:$('#question').value,images,markdown:knowledge.markdown||''})});
...
function renderRealReport(r){
  ...
  $('#result').innerHTML=`<div class="result-heading"><div><h2>${esc(r.title)}</h2><p>${esc(r.workspaceName)} · ${esc(r.transaction.id)} · ${(r.durationMs/1000).toFixed(1)} 秒</p></div><button class="text-button" id="export-real">导出排查单</button></div><div class="section-title"><h2>AI 排查结果</h2><span>${esc(r.ai.model||({disabled:'未调用 AI',failed:'AI 调用失败'}[r.ai.status]||''))}</span></div><div class="ai-text" style="white-space:pre-wrap">${esc(r.ai.text)}</div><p>查询仅覆盖所配置的日志文件，每个文件最多匹配 20 处；关键词匹配仍需核实交易关联。金额及业务状态以业务系统为准。</p><div class="section-title"><h2>日志查询覆盖</h2></div>${r.coverage.map(c=>`<p><b>${esc(c.source)} · ${esc(c.environment)} · ${esc(c.file||'')}</b>：${c.status==='failed'?'查询失败 · '+esc(c.message):'取得 '+c.matches+' 条证据'+(c.truncated?' · 返回内容已截断':'')+(c.omitted?' · '+c.omitted+' 条因报告容量限制未纳入':'')}</p>`).join('')}<div class="section-title"><h2>日志证据</h2><span>${r.evidence.length} 条关键词匹配记录</span></div>${r.evidence.map(e=>`<details class="evidence"><summary><span class="tag">${esc(e.id)}</span><b>${esc(e.source)} · ${esc(e.service)} · ${esc(e.environment)}</b><span>${esc(e.file)}:${e.line}</span></summary><pre>${esc(e.text)}</pre></details>`).join('')}<div class="feedback-bar"><h2>处理反馈</h2><select id="real-feedback">${['未反馈','已解决','需要开发介入','判断不正确'].map(v=>`<option ${r.feedback?.status===v?'selected':''}>${v}</option>`).join('')}</select><textarea id="real-note" maxlength="2000" placeholder="记录最终原因">${esc(r.feedback?.note||'')}</textarea><button class="primary" id="real-save">保存反馈</button><button class="text-button" id="real-retry">重新查询并分析</button></div>`;
  renderScreenshotReport(r);
  renderFollowup(r);
  $('#real-retry').onclick=()=>{$('#transaction-id').value=r.searchQuery??r.transaction.id;$('#question').value=r.question;window.questionImages?.set(r.images);$('#investigate-form').requestSubmit();};
...
function renderHistory(){
  ...
  document.querySelectorAll('[data-report]').forEach(el=>{
    const open=()=>{current=rows.find(r=>r.id===el.dataset.report);view('workbench');renderReport();$('#transaction-id').value=current.searchQuery??current.transaction.id;$('#question').value=current.question;window.questionImages?.set(current.images)};
