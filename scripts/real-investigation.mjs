...
export async function investigate(input,sources,config,emit,{signal,knownHostsFile,search=runServerLogs,model=modelTextStream}={}){
  ...
  if(typeof input.question!=='string'||input.question.length>PayTraceLimits.question||typeof input.markdown!=='string'||input.markdown.length>100000)throw Error('问题或业务文档格式无效或过长');
  ...
  const report={id:randomUUID(),kind:'real',workspaceId:workspace,workspaceName:String(input.workspaceName||workspace).slice(0,100),createdAt:new Date().toISOString(),question,images,searchQuery:query,transaction:{id:query||'文本与截图提问',merchant:'未查询'},evidence,coverage,mode:'服务器日志 · AI 分析',diagnosis:'待核实',title:'日志排查分析',revision:0};
  if(!evidence.length){report.title='用户材料分析';report.mode='用户提供文本／截图 · AI 分析'}
