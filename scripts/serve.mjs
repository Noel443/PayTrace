import {projectJobs,runningTask} from './project-jobs.mjs';
...
import {loadEnv,config,status,analyzeStream} from './ai.mjs';
...
const jobs=projectJobs({getStore:()=>projectStore,saveStore:async next=>{await saveProjects(projectsFile,next);projectStore=next},analyze:analyzeProject});
await jobs.recover();
...
let access=requestAccess({port,publicOrigins,authenticated:!!database});
...
const server=http.createServer(async(req,res)=>{
  ...
  if(database&&pathname.startsWith('/api/')){
    try{
      ...
      if(pathname==='/api/import/browser'||pathname==='/api/workspaces'||pathname==='/api/workspaces/delete'||pathname.startsWith('/api/data/')){
        if(pathname==='/api/workspaces/delete'&&(projectBusy||jobs.busy()||sourcesBusy||active)){json(res,409,{message:'请等待排查或配置操作完成后归档空间'});return;}
  ...
  if(req.url==='/api/workspaces/delete'){
    ...
    if(req.method!=='POST'){json(res,405,{message:'请求方式不支持'});return;}
    if(projectBusy||jobs.busy()||sourcesBusy){json(res,409,{message:'项目或数据源操作尚未结束，请完成后重试删除'});return;}
  ...
  if(req.url?.split('?')[0]==='/api/projects'){
    ...
    projectBusy=true;
    try{
      const input=await bodyJson(req,450000);
      const scope=workspaceKey(input.workspace);await checkWorkspace(scope);
      if(input.action==='analyze'){
        if(!aiConfig.enabled)throw Error('请先配置并启用 AI 模型服务商');
        const markdown=database?(await persistent.handle('/api/data/knowledge','GET',null,scope)).markdown:input.markdown;
        const task=await jobs.start(input.id,scope,aiConfig,markdown,input.totalSeconds??1800);
        json(res,202,{task});
      }else if(input.action==='cancel'){
        json(res,200,{task:await jobs.cancel(input.id,scope,input.taskId)});
      }else if(['save','delete'].includes(input.action)){
        await jobs.serial(async()=>{
          if(projectStore.projects.some(p=>p.id===input.id&&p.workspace===scope&&runningTask(p.task)))throw Error('请先停止该项目分析再修改配置');
          const next=changeProjects(projectStore,input);await saveProjects(projectsFile,next);projectStore=next;
        });
        json(res,200,{projects:publicProjects(projectStore,scope)});
      }else throw Error('不支持的项目操作');
    }catch(e){json(res,400,{message:e.code?'项目操作或保存失败，原有结果保留':e.message})}
    finally{projectBusy=false}return;
  }
  ...
  if(['/api/ai/config','/api/ai/test','/api/ai/providers'].includes(req.url)){
    ...
    try{
      ...
      if(req.url==='/api/ai/test'){
        try{await analyzeStream({...next,timeoutSeconds:60},{report:{question:'这是连接测试，无交易和日志证据，请简短回答已连接，不做交易判断。',evidence:[]},markdown:''},()=>{},{maxTokens:256});json(res,200,{message:'连接成功，已通过分析使用的流式请求验证并收到完整文本。测试未修改配置。',model:next.model});}
  ...
  if(['/api/ai/analyze','/api/ai/analyze/stream'].includes(req.url)&&req.method==='POST'){
    ...
    try{
      const chunks=[];let size=0;
      for await(const chunk of req){size+=chunk.length;if(size>5000000){json(res,413,{message:'请求过大，请缩短文档'});return;}chunks.push(chunk);}
      ...
      }else{const result=await analyzeStream(requestConfig,input,()=>{},{signal:controller.signal});json(res,200,result);}
    }catch(e){
...
server.listen(port,'127.0.0.1',()=>{const listeningPort=server.address().port;access=requestAccess({port:listeningPort,publicOrigins,authenticated:!!database});console.log(`PayTrace: http://127.0.0.1:${listeningPort}`)});
