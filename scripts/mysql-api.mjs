...
export async function listWorkspaces(conn){const [rows]=await conn.execute('SELECT id,name,description,businesses FROM workspaces WHERE deleted_at IS NULL ORDER BY created_at,id');return rows.map(row=>({...row,businesses:decode(row.businesses),custom:row.id.startsWith('ws-'),repository:'',commit:'',services:[],cases:[],flow:[]}))}
export function mysqlApi(db){
  ...
  async function handle(pathname,method,input,scope,actor){
    return db.transaction(async conn=>{
      ...
      if(pathname==='/api/import/browser'){
        ...
        for(const entry of input.workspaces){
          ...
          const [[existing]]=await conn.execute('SELECT id,deleted_at FROM workspaces WHERE id=?',[id]);
      ...
      if(pathname==='/api/workspaces'){
        if(method==='GET')return {workspaces:await listWorkspaces(conn)};
        if(method!=='POST')fail('请求方式不支持',405);
        ...
        else await conn.execute('INSERT INTO workspaces(id,name,description,businesses,created_by) VALUES(?,?,?,?,?)',[id,value.name,value.description,JSON.stringify(value.businesses),actor.id]);
        return {workspace:(await listWorkspaces(conn)).find(w=>w.id===id)};
      }
      workspaceKey(scope);
      const [[workspace]]=await conn.execute('SELECT id FROM workspaces WHERE id=? AND deleted_at IS NULL FOR UPDATE',[scope]);if(!workspace)fail('工作空间不存在或已归档',404);
      if(pathname==='/api/workspaces/delete'){
        if(method!=='POST')fail('请求方式不支持',405);
        if((await listWorkspaces(conn)).length<=1)fail('至少保留一个工作空间');
