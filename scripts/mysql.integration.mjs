...
test('MySQL 8 schema, seed, persistence, transactions, isolation, encryption and HTTP sessions',{skip:!enabled,timeout:60000},async()=>{
  ...
  try{
    ...
    // Apply the additive environment migration twice; legacy spaces and reports stay in test.
    const environmentMigration=await readFile(new URL('../sql/20260922000100_workspace_environments.sql',import.meta.url),'utf8');
    await admin.query(environmentMigration);await admin.query(environmentMigration);
    const [[oldSpace]]=await admin.query("SELECT environment FROM workspaces WHERE id='card'");assert.equal(oldSpace.environment,'test');
    const [[productionCount]]=await admin.query("SELECT COUNT(*) AS n FROM workspaces WHERE environment='production'");assert.equal(productionCount.n,3);
    ...
    child=spawn(process.execPath,['scripts/serve.mjs'],{env:{...process.env,PORT:String(port),DATABASE_CONFIG_FILE:configFile,PAYTRACE_ENV:'test',PUBLIC_ORIGINS:''},stdio:['ignore','pipe','pipe']});
    ...
    const environmentHeaders=environment=>({Cookie:cookie,Origin:base,'Content-Type':'application/json','X-PayTrace-Environment':environment});
    const requestIn=(environment,url,method='GET',body)=>fetch(base+url,{method,headers:environmentHeaders(environment),...(body?{body:JSON.stringify(body)}:{})});
    const [testSpaces,productionSpaces]=await Promise.all(['test','production'].map(async environment=>(await (await requestIn(environment,'/api/workspaces')).json()).workspaces));
    assert.ok(testSpaces.some(w=>w.id==='card'));assert.ok(!testSpaces.some(w=>w.id==='ws-production-card'));
    assert.equal(productionSpaces.length,3);assert.ok(productionSpaces.every(w=>w.id.startsWith('ws-production-')));
    assert.equal((await (await requestIn('production','/api/runtime')).json()).environment,'production');
    assert.equal((await (await requestIn('test','/api/runtime')).json()).switchable,true);
    assert.equal((await requestIn('production','/api/data/investigations?workspace=card')).status,404);
    assert.equal((await requestIn('test','/api/data/knowledge?workspace=ws-production-card')).status,404);
    const prodKnowledge={scanEnabled:false,projects:[],markdown:'production-only knowledge'};
    assert.equal((await requestIn('production','/api/data/knowledge?workspace=ws-production-card','PUT',prodKnowledge)).status,200);
    assert.equal((await (await requestIn('test','/api/data/knowledge?workspace=card')).json()).markdown,'');
    const prodReport={...report,id:'production-fixture',workspaceId:'ws-production-card',environment:'production'};
    assert.equal((await requestIn('production','/api/data/investigations?workspace=ws-production-card','POST',prodReport)).status,200);
    assert.equal((await (await requestIn('production','/api/data/investigations?workspace=ws-production-card')).json()).length,1);
    assert.equal((await requestIn('test','/api/data/investigations/production-fixture?workspace=ws-production-card')).status,404);
    for(const route of ['/api/log-sources','/api/projects'])assert.ok((await requestIn('production',route+'?workspace=card')).status>=400);
    const prodSource={action:'save',workspace:'ws-production-card',name:'isolated production source',service:'trx',environment:'生产环境',host:'127.0.0.1',port:22,username:'fixture',password:'fixture-password',logPath:'/tmp/fixture.log',enabled:false};
    assert.equal((await requestIn('production','/api/log-sources','POST',prodSource)).status,200);
    const productionSources=await (await requestIn('production','/api/log-sources?workspace=ws-production-card')).json();assert.equal(productionSources.sources.length,1);
    assert.equal((await (await requestIn('test','/api/log-sources?workspace=card')).json()).sources.length,0);
    assert.ok((await requestIn('test','/api/log-sources','POST',{...prodSource,id:productionSources.sources[0].id,name:'must not change'})).status>=400);
    assert.equal((await requestIn('production','/api/import/browser','POST',{workspaces:[]})).status,403);
    const created=await (await requestIn('production','/api/workspaces','POST',{name:'production custom workspace'})).json();assert.ok(created.workspace.id);
    assert.equal((await requestIn('test','/api/data/knowledge?workspace='+created.workspace.id)).status,404);
    assert.equal((await requestIn('invalid','/api/workspaces')).status,400);
